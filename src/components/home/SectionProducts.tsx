...
// ✅ 2. إضافة onLoad إلى الدالة مع destructuring
export function SectionProducts({ 
  sectionId, 
  sectionName, 
  showAllSections = false,
  ...
  const [displayCount, setDisplayCount] = useState(initialDisplayCount);
  const [error, setError] = useState<string | null>(null);
  const [isDataLoaded, setIsDataLoaded] = useState(false); // ✅ 3. إضافة isDataLoaded
  const isMounted = useRef(true);

  // ✅ 4. استدعاء onLoad بعد التحميل
  useEffect(() => {
  ...
  // ✅ 5. استدعاء onLoad في حالة الخطأ
  useEffect(() => {
    if (error && !isDataLoaded && onLoad) {
      setIsDataLoaded(true);
      setTimeout(() => {
        onLoad();
      }, 0);
    }
  }, [error, isDataLoaded, onLoad]);

  // ✅ 6. استدعاء onLoad في حالة عدم وجود أقسام
  useEffect(() => {
    if (!isLoading && sections.length === 0 && !isDataLoaded && onLoad) {
      setIsDataLoaded(true);
      setTimeout(() => {
        onLoad();
      }, 0);
    }
  }, [isLoading, sections.length, isDataLoaded, onLoad]);
